                ...
                filesList.forEach(item => {
                    if (item.file) {
                        ...
                        if (fs.existsSync(fullPath)) {
                            ...
                            archive.file(fullPath, { name: `${folderName}/${v.visit_date}_${item.label}_ز${v.visit_id}${ext}` });
                        }
                    }
                });
            });
...
app.post('/api/visit', cpUpload, (req, res) => {
    try {
        const { visitId, patientId: passedPatientId, name, age, phone, visitDate, visitType, customVisitType, diagnosis, customDiagnosis, medicationType, medicationText, hasEcg, hasRays, hasOtherRays, hasLabs } = req.body;
        ...
        if (patientId) {
            ...
            if (existingPatient) {
                ...
                db.prepare('UPDATE patients SET name = ?, age = ? WHERE id = ?').run(cleanName, age, patientId);
            } else {
                const info = db.prepare('INSERT INTO patients (name, age, phone) VALUES (?, ?, ?)').run(cleanName, age, cleanPhone);
...
app.delete('/api/visit/:id', (req, res) => {
    try {
        const visit = db.prepare('SELECT medication_file, ecg_file, rays_file, other_rays_file, labs_file FROM visits WHERE id = ?').get(req.params.id);
        if (visit) {
